document.addEventListener('DOMContentLoaded', () => {
    ...
    async function loadHistory() {
        ...
        if (history.length === 0) {
            historyList.innerHTML = '<div style="color: var(--text-muted); font-size: 11px; text-align: center; margin-top: 20px;">No history.</div>';
            return;
        }
        history.forEach(item => {
            ...
            div.innerHTML = `
                ...
                <button class="btn-icon">Load</button>
            `;
    ...
    async function captureFullPage() {
        try {
            ...
            captureFullBtn.innerHTML = '<div class="loader"></div> Calculating Canvas...';
            ...
            for (let y = 0; y < totalH; y += viewH) {
                ...
                for (let x = 0; x < totalW; x += viewW) {
                    ...
                    const actualPos = await chrome.scripting.executeScript({
                        target: { tabId: tab.id },
                        func: (tx, ty) => {
                    ...
                    captureFullBtn.innerHTML = `<div class="loader"></div> ${progress}% Scanned...`;
    ...
    async function captureVisiblePage() {
        try {
            ...
            captureVisibleBtn.innerHTML = '<div class="loader"></div> Capturing...';
            ...
            chrome.downloads.download({
                ...
                if (chrome.runtime.lastError) showError("Download failed: " + chrome.runtime.lastError.message);
    ...
    // --- HD Video Recording Logic (1080p) ---
    let mediaRecorder;
    let recordedChunks = [];
    ...
    async function startRecording() {
        try {
            // Check if we are in a popup (which closes on interaction)
            if (window.innerHeight < 200) {
                showError("⚠️ Please open in Sidebar or Expand for stable recording.");
            }

            // High-definition 1080p constraints
            const constraints = {
                video: {
            ...
            // MimeType Fallback Logic for maximum compatibility
            const mimeTypes = ['video/webm;codecs=vp9,opus', 'video/webm;codecs=vp8,opus', 'video/webm'];
            const actualMime = mimeTypes.find(m => MediaRecorder.isTypeSupported(m)) || '';

            if (!actualMime) {
                throw new Error("Your browser does not support video recording formats.");
            }

            recordedChunks = [];
            mediaRecorder = new MediaRecorder(stream, { 
                mimeType: actualMime, 
                bitsPerSecond: 6000000 // Stable 1080p bitrate
            });
            ...
            mediaRecorder.onstop = () => {
                ...
                chrome.downloads.download({
                    url: url,
                    filename: `nexus_1080p_${Date.now()}.webm`,
                    saveAs: true
                }, () => {
                    showFeedback('HD Ultra Video Saved!');
            ...
            // Detect if user stops sharing via browser button
            ...
            showFeedback('Nexus HD Recording Started...');
    ...
    function addHeaderRow(k = '', v = '') {
        ...
        row.innerHTML = `<input type="text" class="header-key" value="${k}" placeholder="Key">
                        <input type="text" class="header-value" value="${v}" placeholder="Value">
                        <button class="remove-header" style="background:transparent;border:none;color:var(--text-muted);cursor:pointer;">✕</button>`;
    ...
    async function sendRequest() {
        ...
        sendBtn.innerHTML = '<div class="loader"></div> Processing...';
        ...
        try {
            ...
            await chrome.storage.local.get(['history'], (d) => {
                const h = d.history || [];
